...
function listPkgDirs() {
	go list -f '{{.Dir}}' -tags=ignore_autogenerated,e2e ./... | grep -v generated
	git ls-files labsdk
...
function listFiles() {
	...
	listPkgDirs | xargs -I {} find {} \( -name '*.go' -o -name '*.sh' -o -name '*.py' \)  | grep -v generated | grep -v _pb2
}
...
for file in $allfiles; do
  if ! head -n6 "${file}" | grep -Eq "(Copyright|generated|GENERATED|Licensed)" ; then
    licRes="${licRes}\n  - ${file}"
    if [ ! -z ${GITHUB+x} ]; then
