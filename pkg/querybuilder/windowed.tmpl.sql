{{- /*gotype: github.com/natun-ai/natun/internal/querybuilder.featureQuery */ -}}
{{- /***
  # Windowed feature
  --------------------------
...
WITH
    ...
    base AS (SELECT FQN,
             ...
             WHERE TIMESTAMP BETWEEN {{subtractDuration .Staleness .Since}} AND {{.Until}}
               AND FQN = '{{.FQN}}'
    ),
    data as (SELECT * FROM base WHERE TIMESTAMP >= {{.Since}}),
    {{- /* 2.1. Get the buckets data with start and end dates */}}
    winData_{{tmpName .FQN}} AS (SELECT *, {{subtractDuration .Staleness "timestamp"}} AS WIN_START, TIMESTAMP AS WIN_END
                 FROM base
    ...
    {{tmpName .FQN}} AS (SELECT
           ...
           FROM (SELECT * FROM winData_{{tmpName .FQN}} WHERE WIN_END >= {{$.Since}}) b1
                    LEFT JOIN winData_{{tmpName .FQN}} as b2 ON b1.ENTITY_ID = b2.ENTITY_ID
